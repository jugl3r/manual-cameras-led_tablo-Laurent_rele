...
Предварительно перенесите файл "SADP.exe" (файл находится в репозитории, если не работает, скачайте вручную версию v3.1.2.3.build20240312 с интернета) на сервер, в котором будет производиться настройка устройств 
...
Необходимо в файле по пути go2rtc/go2rtc/go2rtc.yaml заменить IP-адрес и учётные записи на созданные вами при настройке в SADP:
```bash
...
```
Так-же необходимо заменить в конце файла учётные данные от камер на созданные вами при настройке в SADP:
```bash
...
Для добавление камер в ПО "Vizortek" необходимо перейти в главном меню в пункт "Камеры" и нажать на кнопку "Добавить камеру":
![alt text](https://github.com/jugl3r/manual-cameras-led_tablo-Laurent_rele/raw/main/images/vizortek_APP/vizortek_add_camera.png)

После введите необходимую информацию:
![alt text](https://github.com/jugl3r/manual-cameras-led_tablo-Laurent_rele/raw/main/images/vizortek_APP/vizortek_add_camera.png)

Для работоспособности снапшотов в настройках камеры измените параметр "Ссылка на снапшот" на следующий параметр:
```bash
http://192.168.1.100/ISAPI/Streaming/channels/101/picture?snapShotImageType=JPEG
#Подставьте актуальный IP-адрес камеры
```
Для работоспособности изображения трансляции необходимо на роутере пробросить порт 1984 наружу. В настройках камеры измените параметр "Ссылка на трансляцию" на следующий параметр:
```bash
wss://domain-name.ru/wsCams?src=enter&media=video+audio
#Подставьте актуальный домен, если его нет, можно вместо домена поставить ip-адрес сервера с портом 1984
wss://localhost:1984/wsCams?src=enter&media=video+audio
#Параметр "src=" может принимат значения enter и exit, если есть несколько камер, то дополняйте enter_2 и exit_2
``` 
